...
def check_loc(point, lat, longi):
    if point == 'NULL':
        ...
        lstPoint, lstLat, lstLongi = [], [], []
        occ = lstPoint.count('|')
        strPoint = point[1:-1]
        strLat = lat[1:-1]
        strLongi = longi[1:-1]
        #create lists for point, lat & longi
        for i in range(occ):
            #Loop for point
            lstPoint.append(strPoint[:strPoint.find('|')])
            strPoint = strPoint[strPoint.find('|')+1:]
            #Loop for lat
            lstLat.append(strLat[:strLat.find('|')])
            strLat = strLat[strLat.find('|')+1:]
            #loop for longi
            lstLongi.append(strLongi[:strLongi.find('|')])
            strLongi = strLongi[strLongi.find('|')+1:]
        #loop through list and check if same
        result = True
        for p, la, lo in zip(lstPoint, lstLat, lstLongi):
            if p <> la + ' ' + lo:
                result = False
    return result
