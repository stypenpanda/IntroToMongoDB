...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for e in soup.find_all(class_='dataTDRight'):
            if e.name == 'tr':
                for child in e.children:
                    if child.name == 'td':
                        if child.string == 'TOTAL':
                            ...
                            test.append(child.string)
                if checker == 0:
                    info['year'] = int(test[0])
                    info['month'] = int(test[1])
                    info['flights'] = {'domestic': int(test[2].replace(',','')), 'international': int(test[3].replace(',',''))}
                    data.append(info)
                checker = 0
                test = []
    print data
