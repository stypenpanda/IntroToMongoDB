...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            curdict = {}
            #Add label
            curdict['label'] = correct_label(line['rdf-schema#label'])
            #Add URI
            curdict['uri'] = line['URI']
            #Add description
            curdict['description'] = line['rdf-schema#comment']
            #Add name
            if line['name'] == 'NULL' or p.search(line['name']) is None:
                curdict['name'] = curdict['label']
            else:
                curdict['name'] = line['name']
            #Add synonym
            if line['synonym'] <> 'NULL':
                curdict['synonym'] = parse_array(line['synonym'])
            else:
                curdict['synonym'] = None
            #Add classification
            curdict['classification'] = {}
            curdict['classification']['family'] = line['family_label']
            curdict['classification']['class'] = line['class_label']
            curdict['classification']['phylum'] = line['phylum_label']
            curdict['classification']['order'] = line['order_label']
            curdict['classification']['kingdom'] = line['kingdom_label']
            curdict['classification']['genus'] = line['genus_label']
            #Set 'NULL' to None
            for key, value in curdict.iteritems():
                if value == 'NULL':
                    curdict[key] = None
            for key, value in curdict['classification'].iteritems():
                if value == 'NULL':
                    curdict['classification'][key] = None
            data.append(curdict)
            pass
    pprint.pprint(data)
    return data
